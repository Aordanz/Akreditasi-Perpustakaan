                                                ...
                                                @foreach ($sub->dokumenBuktis as $dokumen)
                                                    <div class="flex items-center justify-between gap-3 p-2.5 rounded-xl bg-white border border-slate-200/60 hover:border-[#0a7a3b]/40 hover:bg-green-50/20 transition-all group/file">
                                                        ...
                                                        <form action="{{ route('admin.dokumen.delete', $dokumen->id) }}" method="POST" class="shrink-0" onsubmit="return confirm('Apakah Anda yakin ingin menghapus dokumen ini?');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="w-7 h-7 rounded bg-red-50 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors border border-red-100" title="Hapus Dokumen">
