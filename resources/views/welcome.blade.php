            ...
            <div data-aos="fade-up" data-aos-duration="800" data-aos-delay="{{ $loop->index * 100 }}" class="bg-white rounded-2xl shadow-sm border border-slate-200 border-l-[6px] border-l-[#0a7a3b] overflow-hidden transition-all duration-500 glow-hover"
                ...
                <button @click="openLevel1 = openLevel1 === {{ $komponen->id }} ? null : {{ $komponen->id }}" 
                    ...
                    <div class="flex items-center gap-6 w-full">
                        <!-- 1. Logo -->
                        <div class="w-20 h-20 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-300"
                        ...
                        <div class="flex-1 flex flex-col items-start gap-1">
                            <!-- 2. Nomor -->
                            <div class="bg-green-100 text-[#0a7a3b] px-3 py-1 rounded-lg text-sm font-black border border-green-200 shadow-sm"
                                 :class="openLevel1 === {{ $komponen->id }} ? 'bg-[#0a7a3b] text-white border-[#0a7a3b]' : ''">
                                {{ $komponen->nomor }}
                            </div>
                            
                            <!-- 3. Kata Pertama -->
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block mt-1">{{ __('Komponen') }}</span>
                            
                            <!-- 4. Kata Kedua -->
                            <h2 class="font-black text-xl md:text-2xl transition-colors leading-snug"
                                :class="openLevel1 === {{ $komponen->id }} ? 'text-[#0a7a3b]' : 'text-slate-800 group-hover:text-[#0a7a3b]'">
                                {{ __($komponen->nama_komponen) }}
                            </h2>
                        </div>
                        
                        <div class="w-12 h-12 rounded-full flex items-center justify-center bg-[#e6f4ea] transition-colors group-hover:bg-green-200 shrink-0 border border-green-50 ml-auto">
                            ...
                            <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm transition-all duration-300 relative"
                                ...
                                <button @click="openLevel2 = openLevel2 === {{ $sub->id }} ? null : {{ $sub->id }}" 
                                    ...
                                    <div class="flex items-center gap-5 w-full">
                                        <!-- 1. Logo -->
                                        <div class="w-16 h-16 rounded-xl bg-[#e8f3ec] text-[#0a7a3b] flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-300"
                                        ...
                                        <div class="flex-1 flex flex-col items-start gap-1">
                                            <!-- 2. Nomor -->
                                            <div class="bg-slate-100 text-slate-600 px-2.5 py-0.5 rounded-md text-xs font-bold border border-slate-200"
                                                 :class="openLevel2 === {{ $sub->id }} ? 'bg-green-100 text-[#0a7a3b] border-green-200' : ''">
                                                {{ $sub->nomor_sub }}
                                            </div>
                                            
                                            <!-- 3. Kata Pertama -->
                                            <span class="text-[10px] font-extrabold text-[#0a7a3b] uppercase tracking-widest block mt-0.5">{{ __('Sub Komponen') }}</span>
                                            
                                            <!-- 4. Kata Kedua -->
                                            <h3 class="font-bold text-slate-800 text-lg md:text-xl group-hover:text-[#0a7a3b] transition-colors leading-snug"
                                                :class="openLevel2 === {{ $sub->id }} ? 'text-[#0a7a3b]' : ''">
                                                {{ __($sub->nama_sub_komponen) }}
                                            </h3>
                                        </div>
                                        
                                        <svg class="w-5 h-5 text-slate-400 transform transition-transform duration-300 shrink-0 ml-auto" :class="openLevel2 === {{ $sub->id }} ? 'rotate-180 text-[#0a7a3b]' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                    </div>
                                </button>
